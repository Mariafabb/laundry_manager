...
class OrdiniRepository extends ServiceEntityRepository
{
    ...
    public function findByLikeFilter($filter)
    {
        return $this->createQueryBuilder('o')
            ->select('o.id, c.nome, c.cognome, o.data_ordine, o.data_consegna, o.note, u.cognome as utente, o.totale')
            ->join('o.cliente', 'c')
            ->join('o.user', 'u')
            ->andWhere('c.nome LIKE :val OR c.cognome like :val1 OR o.id LIKE :val2 OR o.data_ordine like :val3 OR o.data_consegna like :val4')
            ->setParameter('val', "$filter%")
            ->setParameter('val1', "$filter%")
            ->setParameter('val2', "$filter%")
            ->setParameter('val3', "$filter%")
            ->setParameter('val4', "$filter%")
            ->orderBy('o.id', 'ASC')
            ->setMaxResults(10)
            ->getQuery()
            ->getResult()
